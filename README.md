...
Model Context Protocol (MCP) server for MySQL — a minimal and highly readable codebase to run SQL queries via stdio (read-only by default). It lets MCP clients run a single SQL statement per invocation.
